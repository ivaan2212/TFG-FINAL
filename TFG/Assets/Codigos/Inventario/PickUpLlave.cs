...
{
    private InventoryLlave inventory;
    ...
    private void Start()
    {
        inventory = GameObject.FindGameObjectWithTag("Player").GetComponent<InventoryLlave>();
    ...
    private void RecogerObjeto()
    {
        ...
        if (Vector2.Distance(transform.position, inventory.transform.position) < 1.5f)
        {
            ...
            for (int i = 0; i < inventory.slotsLlave.Length; i++)
            {
                ...
                if (!inventory.isFull[i])
                {
                    ...
                    GameObject newItem = Instantiate(itemPrefab, inventory.slotsLlave[i].transform.position, Quaternion.identity, inventory.slotsLlave[i].transform);
