...
public class PickUpNotas : MonoBehaviour
{
    private InventoryNotas inventory;
    ...
    private void Start()
    {
        inventory = GameObject.FindGameObjectWithTag("Player").GetComponent<InventoryNotas>();
    ...
    private void RecogerObjeto()
    {
        ...
        if (Vector2.Distance(transform.position, inventory.transform.position) < 1.5f)
        {
            ...
            for (int i = 0; i < inventory.slotsNotas.Length; i++)
            {
                ...
                if (!inventory.isFull[i])
                {
                    ...
                    GameObject newItem = Instantiate(itemPrefab, inventory.slotsNotas[i].transform.position, Quaternion.identity, inventory.slotsNotas[i].transform);
